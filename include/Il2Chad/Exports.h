#pragma once
#include <Il2Chad/Structs.h>
#include <vector>

#define IL2CPP_API(ret, name, ...)                                                                                                                                                                     \
	typedef ret (*name##_t)(__VA_ARGS__);                                                                                                                                                              \
	inline name##_t name = (name##_t) Internal::registerExport(#name, (void **)&name);

namespace Il2Chad::Exports
{
	namespace Internal
	{
		struct Export
		...
		inline std::vector<Export> exports;

		inline void* registerExport(const char* name, void** functionPtr)
		{
			exports.push_back({name, functionPtr});
			return nullptr;
		}
	}
	...
	// This whole thing is just a quality of life measure so it may just be easier to have people specify assembly names
	// manually
